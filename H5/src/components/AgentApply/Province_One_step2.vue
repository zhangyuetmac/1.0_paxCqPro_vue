<template>
  <div class="MerchantApply" :style="mainBackground" @touchstart="myTouchStart" @touchend="myTouchEnd">
    <div class="head">
      <img src="./image/icon_arrowleft1.png" @click="goBack">
      <a>企业一级代理商</a>
    </div>
    <div class="step">
        <div class="item">
    ...
    <div class="main">
      <div class="mainTop" ref="mainTopWrapper">
        <ul class="mainTopUl">
          ...
          <li class="inputLi">     
            <a>开户支行</a>
            <input class="myInput" type="text"  placeholder="点击输入支行"   v-model="merchantApply1.merchant_name"/>
          </li >
...
export default {
  ...
  methods: {
    ...
    //弹出行业类目category_id二级选择
    toastLeimu() {
      this.showLeimu = true;
      this._inputBlur();
    },
    //选择行业类目时候滑动时候触发的onChange函数
    ...
    onConfirm(value, index) {
      ...
      this.myCategory_id = citys[value[0]].name[index[1]];
      this.merchantApply1.category_id = citys[value[0]].code[index[1]];
      this.showLeimu = false;
    },
    //信息验证
    checkData() {
      return true;
  ...
  created() {
    ...
    this.$store.commit(
      "setMerchantApplyEntitity",
      defaultMerchantApplyEntitity
    );
    //设置高度，点击输入框的时候页面不会变形
