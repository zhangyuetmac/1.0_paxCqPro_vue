<template>
  <div class="forgetPassword1" :style="{height:bgHeight}" @touchstart="myTouchStart" @touchend="myTouchEnd">
    <ForgetPasswordHeader></ForgetPasswordHeader> 
...
// import {getRefreshToken} from '../../common/js/loginAndRegister/getRefreshToken.js'
  ...
  methods:{
    ...
    _getPassweord(){
      ...
      const getPasswordParam = {
        ...
        smsCode: this.$store.state.smsCode     
      } 
      // const getPasswordUrl = 'http://cqdev.paxpay.cn/gateway/stock/user/'+this.$store.state.mobileNumber
      // const getPasswordUrl = this.BASE_URL+'stock/user/'+this.$store.state.mobileNumber+this.$store.state.loginMessage.access_token
      const getPasswordUrl = this.BASE_URL+'cs/stock/user/'+this.$store.state.mobileNumber
      console.log(getPasswordUrl)
      axios.patch(getPasswordUrl, getPasswordParam).then((res) => {
        console.log("忘记密码获取验证码返回的数据==="+JSON.stringify(res));
