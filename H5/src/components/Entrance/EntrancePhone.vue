<template>
  <div class="entranceWrapper" :style="{height:bgHeight}" @touchstart="myTouchStart" @touchend="myTouchEnd">
    ...
    <div class="loginWrapper">
        <!-- <div v-show="inputTransType==='user'" class="user"> 
          <img class="user-img" src="./image/icon_user.png">
          <input class="myInput" type="text" placeholder="请输入账号" v-model="userVal">
        ...
        <!-- <div v-show="inputTransType==='user'" class="password">
          <img class="user-img" src="./image/icon_password.png">
          <input class="myInput" :type="isPassword?'password':'number'" placeholder="请输入密码" v-model="passwordVal">
        ...
        <div class="forgetAndPhone">
          ...
          <span @click="changeLoginType">账号登录</span>
        </div>
    ...
    <!-- <div v-show="messageMistake" class="toastWrapper">
      <van-loading size="24px" type="spinner" vertical>登陆中</van-loading>
    </div>    -->
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    _getLogin() {
      ...
      const loginUrl = this.BASE_URL+"cs/oauth/mobileToken"
      const formData = new FormData()
      formData.append('phoneNo',loginParam.phoneNo)
      formData.append('smsCode',loginParam.smsCode)
      ...
      /*axios.post(loginUrl, formData,{
          ...
          console.log(
            "验证码登录---post访问公网接口返回的数据===" + JSON.stringify(res)
          );
    ...
    _getCodeNet() {
      ...
      //登录时候获取验证码的类型type为2
      getCodeAxios(getCodeParam,2)
      // const getCodeUrl = 'http://cqdev.paxpay.cn/cs/stock/sms/'+this.userVal+"?"+'type=1'
      // const getCodeUrl = this.BASE_URL+'cs/stock/sms/'+this.userVal+"?"+'type=2'
